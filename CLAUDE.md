...
**Filipa** is a smart interview assessment tool for IT developer candidates. The name comes from Czech expression meaning "someone who is smart and knows how to handle situations". This is a Svelte 5 + TypeScript application designed to run in offline/regulated environments with dual-view interface for interviewer and candidate.
...
- use pure functions
- markdown structure for import, export is defined in `src/public/config.json` - must be updated with each object structure change 
...
After `npm run build`, the `dist/` folder can be:
- Opened directly in browser (double-click `index.html`) - should contain all js and css
